...
Which is efficient in space, but not useful for analyses. For many analyses you will need to refer to multiple columns. For example is the total number of people (both males and females) in 2012 different from the total in 2015 in the regular bachelor propedeuses? The format of the raw file is  also called the wide format.  
...
```
When we look at the tail end of the data.frame we see some weird cases. empty and summary variables.
...
Again this is temporary file. Only when we are happy with the endresult wil we save the
```r
...
View(duo2015_tidy)
```
We left out the View command, because that would show it to the Viewer, but we need a new data.frame. The last command `arrange`, does not really change the file, but orders the rows in a different way.
The next step could be saving it to a new csv file. But there is something bothering me. 
